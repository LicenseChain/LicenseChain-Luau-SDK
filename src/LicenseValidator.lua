...
function LicenseValidator:validate(licenseKey, useCache)
	...
	local hwuid = self.client:getHardwareId()
	local body = { key = licenseKey }
	if hwuid and hwuid ~= "" then body.hwuid = hwuid end
	local success, result = self.client:_makeRequest("POST", "/licenses/verify", body)
...
--[[
	...
	Does not use the validator cache or format gates from :validate().
]]
function LicenseValidator:verifyWithDetails(licenseKey)
	if not licenseKey or type(licenseKey) ~= "string" then
		return false, Error.new(Error.Types.INVALID_INPUT, "License key is required")
	end
	local hwuid = self.client:getHardwareId()
	local body = { key = licenseKey }
	if hwuid and hwuid ~= "" then
		body.hwuid = hwuid
	end
	return self.client:_makeRequest("POST", "/licenses/verify", body)
end
...
function LicenseValidator:validateLicenseData(licenseData)
	...
	-- Normalize API response from /licenses/verify into legacy shape
	local normalized = {
		status = tostring(licenseData.status or ""),
		expires = licenseData.expires,
		hardwareId = licenseData.hardwareId,
		user = licenseData.user
	}

	if normalized.status == "" and licenseData.valid == true then
		normalized.status = "active"
	end
	if normalized.expires == nil and licenseData.expiresAt then
		normalized.expires = tonumber(licenseData.expiresAt) or nil
	end
	if normalized.user == nil and licenseData.email then
		normalized.user = licenseData.email
	end

	local statusLower = string.lower(normalized.status)
	if statusLower == "active" then
		normalized.status = "active"
	elseif statusLower == "revoked" then
		normalized.status = "revoked"
	elseif statusLower == "suspended" then
		normalized.status = "suspended"
	elseif statusLower == "expired" then
	...
	-- Check expiration
	if normalized.expires and type(normalized.expires) == "number" then
		local currentTime = os.time()
		if currentTime > normalized.expires then
			return {
				valid = false,
	...
	if normalized.hardwareId and normalized.hardwareId ~= self.client:getHardwareId() then
		return {
			valid = false,
