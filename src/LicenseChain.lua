...
local function normalizeBaseUrl(baseUrl)
	local value = type(baseUrl) == "string" and string.gsub(baseUrl, "/+$", "") or ""
	if value == "" then
		return "https://api.licensechain.app/v1"
	end
	if string.sub(value, -3) == "/v1" then
		return value
	end
	return value .. "/v1"
end

local function normalizeEndpoint(baseUrl, endpoint)
	local baseHasV1 = string.sub(baseUrl, -3) == "/v1"
	if string.sub(endpoint, 1, 4) == "/v1/" then
		return baseHasV1 and string.sub(endpoint, 4) or endpoint
	end
	if string.sub(endpoint, 1, 1) == "/" then
		return baseHasV1 and endpoint or ("/v1" .. endpoint)
	end
	return baseHasV1 and ("/" .. endpoint) or ("/v1/" .. endpoint)
end
...
function LicenseChainClient.new(config)
	...
	self.config = {
		...
		baseUrl = normalizeBaseUrl(config.baseUrl or "https://api.licensechain.app/v1"),
...
function LicenseChainClient:updateLicense(licenseKey, updates)
	...
	return self:_makeRequest("PATCH", "/licenses/" .. licenseKey, updates)
end
...
function LicenseChainClient:extendLicense(licenseKey, days)
	...
	local data = {
		expiresAt = os.date("!%Y-%m-%dT%H:%M:%SZ", os.time() + (days * 86400))
	}
	
	return self:_makeRequest("PATCH", "/licenses/" .. licenseKey .. "/extend", data)
end
...
function LicenseChainClient:getAnalytics(timeRange)
	...
	return self:_makeRequest("GET", "/licenses/stats" .. queryString)
end
...
function LicenseChainClient:_makeRequestWithRetry(method, endpoint, headers, data)
	...
	for attempt = 1, self.config.retries do
		local callSuccess, requestSuccess, requestResult = pcall(function()
			local url = self.config.baseUrl .. normalizeEndpoint(self.config.baseUrl, endpoint)
